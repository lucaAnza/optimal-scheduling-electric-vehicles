...
% the base load for a micogrid  (scale the base load values)
Scale_factor=1/1500;
L_b_mic=L_b*Scale_factor;  % the base load
P_L_b_mic=P_L_b*Scale_factor;  % the predicted base load

%%%%%%% 2-TIME_INTERVAL %%%%%%%%%
tau=1; % in hours (the lengh of charging interval)
num_slot=length(L_b_mic); % number of charging intervals
...
P_max=5; % in KW (% the max charging rate)

%%%%%%% 5- CHG and V2G %%%%%%%%%
num_EV=200; % number of EVs
P_Chg=0; % the percentage of EVs that charge the battery only
num_CHG_EV=round(P_Chg*num_EV);  % the CHG EVs will be in the front part of the EV info matrix. (% number of CHG EVs)
num_V2G_EV=num_EV-num_CHG_EV; % number of V2G EVs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 3-Global optimal scheme for V2G using CVX tool %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% plot the base load
xx_1=1:num_slot;
figure;
yy_1(:,1)=L_b_mic;
...
ylabel('Load [KW]');
xlabel('Hour No.');
legend('Real base load','Predicted base load','Location','Best');


%%%%%%%% 1-the equality constraint: Ax=b
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 4- CVX tool Global schedule solution %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 5- Verify and Organizzation Global schedule solution %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%% 1.verify if all the EVs have been fully charged
...
%%%%%%% 2.verify the charging load at each interval
...
%%%%%%% 3.verify the energy level at each interval for each EV
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 6- The discrete-time optimization loop %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% EV_Matrix (COMPOSITION): 
    % col. 1=EV No.
    % col2=initial energy
    % col3=current energy
    % col4=starting slot No.
    % col5=ending slot No.
    % col6=status (0-not started,% 1-ongoing, 2-completed)
    % col.7=CHG or V2G (0=CHG, 1=V2G), 
    % col. 8=group ID(1, 2, 3...)

% EV_Matrix (DEFINITION): 
EV_Matrix=zeros(num_EV,6);  % status
...
%%%%% 2. Create the 8th column(group_id) into EV_MATRIX_1 --> (EVs_id , init_energy , ... ,  group_id)
...
%%%%% 3. RE-Construction of EV_Matrix (using EV_MATRIX1)
for gg=1:group_num
...
%%%%% 4. Optimization for each time_slot using current EV    
    for iii=1:num_slot   % 1 -> 24

        Current_T=iii;  % the current time instant
        Par_set=[beta, tau, Cap_battery_org, gamma, P_max, omega, num_CHG_g, k_0, k_1]';  % parameter vector

        % find out the charging EVs at the current time using
        Current_EV=[];
        temp_d1=0;
        for i=1:num_EV_g
        ...
        % save data for test --->  save T_Par_set.txt Par_set -ascii; save T_Current_EV.txt Current_EV -ascii; save T_Current_slot.txt Current_slot -ascii;
        ...
        if isempty(Current_EV)
            ...
            % the sliding window
            Current_slot=[];
            Current_slot=Current_T: max(Current_EV(:,5));  % Current slow is from CURRENT_TIME(iii) --> The farthest depaturtime time of active EVs
    ...
    %%%%% 5. Final verification and update  
...
ylabel('Load [KW]');
xlabel('Hour No.');
legend('Base load without EV charging', ...
       'Total load with globally optimal EV charging', ...
       'Total load with locally optimal EV charging', ...
       'Total load with naive EV charging', ...
       'Location', 'NorthWest');
...
ylabel('Energy [KWH]');
xlabel('Hour No.');
legend('EV1','EV2','EV3','EV4','EV5','EV6','EV7','EV8','EV9','EV10','Location', 'NorthWest');
